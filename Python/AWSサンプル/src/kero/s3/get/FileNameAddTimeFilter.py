...
def s3list(s3, bucket_name, prefix="", date_start=None, date_end=None):
    ...
    if "Contents" in response:
        for obj in response["Contents"]:
            ...
            logger.debug(f'{obj["Key"]}: {last_modified}')
            if date_start:
                if last_modified.date() < date_start.date():
                    logger.debug("UnMach.")
                    continue
            if date_end:
                if last_modified.date() > date_end.date():
                    logger.debug("UnMach.")
                    continue
            logger.debug("Mach!")
            list.append(obj["Key"])
